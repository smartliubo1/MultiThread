...
namespace AsyncFile
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Task task = ProcessAsynchronousIO();
        ...
        {
            using (var stream=new FileStream("test1.txt",FileMode.Create,FileAccess.ReadWrite,
                FileShare.None,BUFFER_SIZE))
            {
                Console.WriteLine($"1. Use I/O Threads:{stream.IsAsync}");
            ...
            {
                Console.WriteLine($"2. Use I/O Threads:{stream.IsAsync}");
            ...
            using (var stream = File.Create("test3.txt",
                BUFFER_SIZE, FileOptions.Asynchronous
                ))
            {
                using (var sw=new StreamWriter(stream))
                {
                    Console.WriteLine($"3. Use I/O Threads:{stream.IsAsync}");
            ...
            {
                Console.WriteLine($"4 Use I/O Threads:{((FileStream)sw.BaseStream).IsAsync}");
                await sw.WriteAsync(CreateFileContent());
            }
            ...
            for (int i = 0; i < 4; i++)
            {
                deleteTasks[i] = SimulateAsynchronousDelete($"test{i+1}.txt");
            }
        ...
        static string CreateFileContent()
        {
            ...
            {
                sb.AppendFormat($"{new Random(i).Next(0,99999)}");
                sb.AppendLine();
            }
            return sb.ToString();
        }
